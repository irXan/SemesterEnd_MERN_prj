...
export default function ExercisesPage() {
  ...
  const handleDelete = async (id) => {
    ...
    const res = await deleteExercise(token, id);
    if (res.ok) {
      showSuccess("Deleted");
      loadExercises();
    } else {
  ...
  const handleSubmit = async () => {
    ...
    let res;
    if (editingExercise) {
      res = await updateExercise(token, editingExercise._id, { name });
    } else {
      res = await createExercise(token, { name });
    }

    if (res.ok) {
      showSuccess(editingExercise ? "Updated" : "Created");
      setShowModal(false);
      loadExercises();
    } else {
      showError(res.message);
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-white text-2xl font-black">
            Exercises <span className="text-yellow-500">Manager</span>
          </h1>

          <button
            onClick={openAddModal}
            className="flex items-center gap-2 bg-yellow-500 text-black px-5 py-3 rounded-xl font-bold w-[170px]"
          >
            <Plus /> Add Exercise
          </button>
        </div>
          ...
          <table className="w-full text-sm text-left text-gray-400">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="2" className="p-6 text-center">No exercises</td></tr>
              ) : (
                filteredExercises.map((e) => (
                  <tr key={e._id} className="border-t border-white/5">
                    <td className="p-4 text-white font-bold">{e.name}</td>
                    <td className="flex gap-2 p-3">
                      <Pencil onClick={() => openEditModal(e)} className="w-4 h-4 text-blue-400 cursor-pointer" />
                      <Trash2 onClick={() => handleDelete(e._id)} className="w-4 h-4 text-red-400 cursor-pointer" />
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center">
          <div className="bg-[#111] p-6 rounded-xl w-96">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={handleSubmit} className="bg-yellow-500 px-4 py-2 rounded">
                Save
              </button>
            </div>
          </div>
