...
import { getMealTypes } from "../services/mealService"; 
...
export default function AddNutritionPage() {
  ...
  const loadMealTypes = useCallback(async () => {
    ...
    console.log("Meal Types Response:", res);
    if (res.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await createNutritionEntry(token, payload);
        ...
        <div className="mx-auto">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-white text-2xl font-black">
              Add <span className="text-yellow-500">Nutrition</span>
            </h1>
            <button
              onClick={() => navigate("/nutrition")}
              className="flex items-center  gap-2 px-4 w-100px py-2 bg-white/5 border border-white/10 rounded-xl text-xs font-bold text-gray-400 hover:text-white"
            >
          ...
          >
            ...
            <button
              className="w-full bg-yellow-500 text-black py-4 rounded-xl font-black uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-white transition"
            >
