...
const PASSWORD_REGEX = /^(?=.{6,100}$)\S.*\S$|^\S{6,100}$/;
...
const LoginPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!PASSWORD_REGEX.test(trimmedPassword)) {
      setError("Password must be 6 to 100 characters with no spaces at the start or end.");
      return;
    }
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-10 rounded-3xl shadow-2xl">
          ...
          {location.state?.successMessage && (
            <div className="mb-6 p-4 bg-green-500/20 border border-green-500/30 rounded-2xl">
              <p className="text-green-400 text-xs font-bold uppercase tracking-wider text-center">
                {location.state.successMessage}
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={handleChange}
                required
                pattern="^(?=.{6,100}$)\S.*\S$|^\S{6,100}$"
                title="Password must be 6 to 100 characters with no spaces at the start or end."
